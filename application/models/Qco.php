...
class Application_Model_Qco
{
	...
	*/
	...
	{
		$qco = new Application_Model_DbTable_Qco();
		return $qco->fetchAll($qco->select()->where('number_communication = ?',$field)->where('end_date IS NULL')->order('number_communication'));
	}
	...
	*/
	...
	/**
	*	Return route data of QCO by type journey.
	*
	*	@param integer $qcoId -> QCO's id
	*	@param integer $typeJourney -> type journey id
	*	@access public
	*	@return Zend_Db_Table_Rowset
	*/
	public function returnRoute($qcoId,$typeJourney)
	{
		$qco = new Application_Model_DbTable_QcoRoute();
		$select = $qco->select()->setIntegrityCheck(false);
		$select	->from(array('q' => 'qco_route'))
						->where('q.qco_id = ?',$qcoId)
						->where('q.type_journey = ?',$typeJourney);
		return $qco->fetchAll($select);
	}

	/**
	*	Return schedule data of QCO.
	*
	*	@param integer $qcoId -> QCO's id
	*	@access public
	*	@return Zend_Db_Table_Rowset
	*/
	...
	{
		$qco = new Application_Model_DbTable_QcoSchedule();
		$select = $qco->select()->setIntegrityCheck(false);
		$select	->from(array('q' => 'qco_schedule'))
						->joinInner(array('qt' => 'qco_type_day'),'qt.id=q.type_day_id',array('type_day_name' => 'name'))
						->where('q.qco_id = ?',$qcoId)
 						->order('q.type_day_id');
		return $qco->fetchAll($select);
	}
	...
	*/
	...
	/**
	*	Check if QH file is correct and it belongs to the right line.
	*
	*	@param array $files -> QH's data file
	*	@param array $data -> QCO's data
	*	@access public
	*	@return boolean
	*/
	public function checkQH($files,$data)
	{
	...
	/**
	*	Clone a QCO with certain id with route data.
	*
	*	@param integer $qcoId -> qco's id
	*	@access public
	*	@return integer
	*/
	public function cloneQCO($qcoId)
	{
		$qco = new Application_Model_DbTable_Qco();
		$mainRow = $qco->fetchRow($qco->select()->where('id = ?', $qcoId));
		$mainRow->end_date = new Zend_Db_Expr('NOW()');
		$mainRow->save();

		$data = $mainRow->toArray();
		unset($data['id']);
		$data['start_date'] = new Zend_Db_Expr('NOW()');
		$data['end_date'] = new Zend_Db_Expr('NULL');
		$newMainQCO = $qco->createRow($data);
		$newQcoId = $newMainQCO->save();

		$qcoRoute = new Application_Model_DbTable_QcoRoute();
		$routeRow = $qcoRoute->fetchAll($qcoRoute->select()->where('qco_id = ?', $qcoId));
		foreach ($routeRow as $route) 
		{
			$dataRoute = $route->toArray();
			unset($dataRoute['id']);
			$dataRoute['qco_id'] = $newQcoId;
			$newRouteQCO = $qcoRoute->createRow($dataRoute);
			$newRouteQCO->save();
		}
		return $newQcoId;
	}
