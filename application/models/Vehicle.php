...
class Application_Model_Vehicle
{
	...
	*/
	...
	{
		if($this->checkedExternalNumber($data['external_number']))
		{
			...
			$data['user_id'] = $authNamespace->user_id;
 			$data['plate'] = preg_replace('/\s+/','',strtoupper($data['plate']));
			$data['floor'] = 1;
...
/**
	...
	*/
	protected function saveMain($data,$vehicleId)
	{
		...
		$change_crv = 2;
		$plate = preg_replace('/\s+/','',strtoupper($data['plate']));
		if($vehicleRow)
		{
			if($vehicleRow->plate != $plate || $vehicleRow->renavam != $data['renavam'] || $vehicleRow->color != $data['color']){
				$change1=1;
			}
			else{
				$change1=2;
			...
			if($edit){
				...
				if($vehicleHistoricRow){
					if($vehicleHistoricRow->external_number != $data['external_number']){
						// colocar data de baixa no historico aqui
						$vehicleHistoricRow->end_historic_date = new Zend_Db_Expr('NOW()');
						$vehicleHistoricRow->save();
						
						$datah['start_historic_date'] = new Zend_Db_Expr('NOW()');
						$datah['end_historic_date'] = NULL;
						$datah['external_number'] = $data['external_number'];
						$datah['authorization'] = NULL;
						$datah['vehicle_id'] = $vehicleId;
						$datah['consortium_company'] = $vehicleHistoricRow->consortium_company;
						$datah['consortium'] = $vehicleHistoricRow->consortium;
						$vehicleHistoricNew = $vehicleHistoric->createRow($datah);
						$vehicleHistoricNew->save();
	...
	*/
	...
	{
		try{
			if($this->checkedExternalNumber($data['external_number'],$vehicleId))
			{
				...
				if($authNamespace->institution == 3){
					...
					if($authNamespace->company != ""){ // if the consortium is registering there is no company.
	...
	public function saveLog($vehicleId, $status,$data)
	{
  	$authNamespace = new Zend_Session_Namespace('userInformation'); 
		...
		$vehicleNew->plate = preg_replace('/\s+/','',strtoupper($data['plate']));
	...
	public function returnById($vehicleId)
	{
		...
		$select	->from(array('v' => 'vehicle'),array('st'=>'start_date', 'start_date' => new Zend_Db_Expr ('DATE_FORMAT(start_date,"%d/%m/%Y")'),
																						'service', 'plate', 'renavam', 'pattern', 'color', 'type', 'floor') )
						->joinLeft(array('m' => 'vehicle_mechanics'),'m.id=v.id')
						->joinLeft(array('me' => 'vehicle_measures'),'me.id=v.id',array('number_seats'=>'amount_seats'))
						...
						->joinLeft(array('com' => 'company'),'com.id=co.company',array('company_name' => 'company','company_id' => 'id'))
        ...
        public function returnLastInspection($vehicleId){
		$vehicle = new Application_Model_DbTable_Vehicle();
		$select = $vehicle->select()->setIntegrityCheck(false);
		$select	->from(array('vi' => 'vehicle_inspection'),array('data'=>new Zend_Db_Expr('max(date_inspection)')))					
         ...
         public function returnLastCRV($vehicleId){
		$vehicle = new Application_Model_DbTable_Vehicle();
		$select = $vehicle->select()->setIntegrityCheck(false);
		$select	->from(array('vac' => 'vehicle_ask_crv'),array('id'=>new Zend_Db_Expr('max(id)')))					
						->where('vac.vehicle_id = ?',$vehicleId);        
		...
		if($institution == 3)
		{
			if($company)
			{
				...
				$select	->from(array('v' => 'vehicle'),array('start_date' => new Zend_Db_Expr ('DATE_FORMAT(start_date,"%d/%m/%Y")'),
								...
								->where('m.status = 4 OR m.status = 1 OR m.status = 8 OR m.status = 14')
    ...
    $company = $authNamespace->company;
        ...
        public function returnAvgYearChassi()
        {
            try{
                ...
                $vehicle = new Application_Model_DbTable_Vehicle();
		$select = $vehicle->select()->setIntegrityCheck(false);
		$select	->from(array('v' => 'vehicle'))
						->joinInner(array('vm' => 'vehicle_mechanics'), 'v.id=vm.id',array('chassi_life_cycle' => new Zend_Db_Expr("ROUND(AVG(".date('Y')." - chassi_year))"),'chassi_count' => "Count('chassi_year')",'body_life_cycle' => new Zend_Db_Expr("ROUND(AVG(".date('Y')." - body_year))")))                                               
						->joinLeft(array('vh' => 'vehicle_historic'),'vh.vehicle_id=v.id',array())
    ...
    $company = $authNamespace->company;
		if($institution == 3)
		{
			if($company)
			{
				//tirei o group by deste select ->group('vh.external_number')
				$vehicle = new Application_Model_DbTable_Vehicle();
				$select = $vehicle->select()->setIntegrityCheck(false);
				$select	->from(array('v' => 'vehicle'),array('start_date' => new Zend_Db_Expr ('DATE_FORMAT(start_date,"%d/%m/%Y")'),
																								'service', 'plate', 'renavam', 'pattern', 'color', 'type', 'floor', 'vehicle_id' => 'id') )
	...
	{
		//tem que pegar aqui os que estão aguardando a aprovação final, então tem q estar com status 2 na vehicle_new
		$vehicle = new Application_Model_DbTable_Vehicle();
		$select = $vehicle->select()->setIntegrityCheck(false);
		$select	->from(array('v' => 'vehicle'),array('vehicle_id' => 'id', 'plate', 'renavam') )
						...
						->joinInner(array('vn' => 'vehicle_new'),'v.id=vn.vehicle_id')
						->where('vq.status = 12')
						->where('vn.status = 2')
						->group('v.id');
		return $vehicle->fetchAll($select);
		...
		try{
			...
			if($this->saveFile($files,$ext,$data,$vehicleId))
			{
				...
				if($data['date_notification'] != ''){
					...
					$aux = explode(':',$data['hour_notification']);
					$vehicleNew->document = $data['document'].'_'.$date.'_'.$aux[0].'-'.$aux[1].'.'.$ext;
				}
				else if($data['date_inspection'] != ''){
					$date = Application_Model_General::dateToUs($data['date_inspection']);
					$aux = explode(':',$data['hour_inspection']);
					$vehicleNew->document = $data['document'].'_'.$date.'_'.$aux[0].'-'.$aux[1].'.'.$ext;
					// buscar aqui se o veiculo ja tem laudo de vistoria, se ja tiver considera como vistoria programada
					// se não considera que é uma inclusão
					$newVehicle = array_filter($this->returnInspectionData($vehicleId));
					if(!empty($newVehicle)){
						//echo"laudo de vistoria programada";
					}
					else{
						//echo"este laudo de vistoria é de vistoria de inclusão de novo veículo";
						$this->changeStatus($vehicleId,12);
					}
					$this->changeStatusVehicleNew($vehicleId,2);
				}
				else{
					$date = date('Y-m-d');
	...
	*/
	protected function saveFile($file,$ext,$data,$vehicleId)
	{
		...
		if($data['document'] == 'inspection'){
			$date = Application_Model_General::dateToUs($data['date_inspection']);
			$aux = explode(':',$data['hour_inspection']);
			$newFile = $target_directory.'/'.$data['document'].'_'.$date.'_'.$aux[0].'-'.$aux[1].'.'.$ext;
		}
		else if($data['document'] == 'notification'){
			$date = Application_Model_General::dateToUs($data['date_notification']);
			$aux = explode(':',$data['hour_notification']);
			$newFile = $target_directory.'/'.$data['document'].'_'.$date.'_'.$aux[0].'-'.$aux[1].'.'.$ext;
		}
	...
	*/
	...
	public function newCRV($vehicleId)
	{
        ...
        if(!$vehicle_new && $authNamespace->institution != 1){
	...
	{
		// se mudar o status para 4(rodando) tem que testar se vai solicitar segunda via de CRV, 
		// isso deve acontecer em casos de edição.
		if($status == 4){
	...
	*	@return boolean
	**/
	public function vehicleIsNew($vehicleId){
	...
	/**
	*	Change the vehicle_new status to inative
	*
	*	@param integer $vehicleId - vehicle's id
	*	@access public
	*	@return integer
	**/
	public function changeStatusVehicleNew($vehicleId,$status)
	{
		$vehicleStatus = new Application_Model_DbTable_VehicleNew();
		$vehicleStatusRow = $vehicleStatus->fetchRow($vehicleStatus->select()->where('vehicle_id = ?',$vehicleId));
		$vehicleStatusRow->status = $status;
	...
	*	@return integer
	*/
	public function saveInspection($data,$vehicleId)
	{
		try{
			$authNamespace = new Zend_Session_Namespace('userInformation');
			$vehicle = new Application_Model_DbTable_VehicleInspection();
	...
	*/
	...
	public function vehiclesAskedCrv()
	{
		...
		$select	->from(array('vt' => 'vehicle_ask_crv'),array('id', 'justify') )
						->joinInner(array('v' => 'vehicle'),'v.id=vt.vehicle_id',array('vehicle_id' => 'id','plate'))
						->joinInner(array('vh' => 'vehicle_historic'),'v.id=vh.vehicle_id AND end_historic_date IS NULL',array('historic_id' => 'id'))
						->where('vt.status = 2 OR vt.status=1')
						->group('vt.vehicle_id');
		return $transfer->fetchAll($select);
	}
	...
	*	@return integer
	*/
	public function returnDocument($vehicleId,$document,$hour=0){
		if($hour != 0){
			$hour = explode(":",$hour); 
			$document = $document.'_'.$hour[0]."-".$hour[1];
		}
		$aux = substr($document,0,4);
		$aux2 = explode("_",$document);

		$vehicle = new Application_Model_DbTable_VehicleDocuments();
		$select = $vehicle->select()->setIntegrityCheck(false);
		$select2 = $vehicle->select()->setIntegrityCheck(false);

		if($aux == 'insp'){
			$registerOld=1;
			if($aux2[2] != ''){				
				$select2	->from(array('v' => 'vehicle_documents'),array('id','document') )
									->where('v.vehicle_id = ?', $vehicleId)
									->where('substr(v.document,1,4) like ?', $aux)
									->where('substr(v.document,12,10) like ?', $aux2[1])
									->where('substr(v.document,23,5) like ?', $aux2[2]);
				$registerOld = $vehicle->fetchRow($select2);
			}
			if($aux2[2] =='' || $registerOld == ''){
		...
		elseif ($aux == 'noti') {
			...
			if($aux2[2] != ''){
				$select2	->from(array('v' => 'vehicle_documents'),array('id','document') )
								->where('v.vehicle_id = ?', $vehicleId)
								->where('substr(v.document,1,4) like ?', $aux)
								->where('substr(v.document,14,10) like ?', $aux2[1])
								->where('substr(v.document,25,5) like ?', $aux2[2]);
				$registerOld = $vehicle->fetchRow($select2);
			}
			if($aux2[2] =='' || $registerOld == ''){
				$select	->from(array('v' => 'vehicle_documents'),array('id','document') )
			...
			if($aux2[2] =='' || $registerOld == ''){
				...
				$select	->from(array('v' => 'vehicle_documents'),array('id','document') )
							->where('v.vehicle_id = ?', $vehicleId)
							->where('substr(v.document,1,4) like ?', $document);
                                                        echo $select;
			}
			else{
				$select	->from(array('v' => 'vehicle_documents'),array('id','document') )
			...
			$select	->from(array('v' => 'vehicle_documents'),array('id','document') )
                ...
                $vehicleRow = $vehicle->fetchAll($select);
	...
	*/
	...
	public function vehiclesDown()
	{
		$vehicle = new Application_Model_DbTable_Vehicle();
		$select = $vehicle->select()->setIntegrityCheck(false);
		$select	->from(array('v' => 'vehicle'),array('start_date' => new Zend_Db_Expr ('DATE_FORMAT(start_date,"%d/%m/%Y")'),
																						'service', 'plate', 'renavam', 'pattern', 'color', 'type', 'floor', 'vehicle_id' => 'id') )
						->joinInner(array('vh' => 'vehicle_historic'),'vh.vehicle_id=v.id AND vh.end_historic_date IS NULL',array('historic_id' => 'id','external_number'))
						->joinInner(array('c' => 'consortium'),'vh.consortium=c.id',array('consortium_name' => 'name'))
						->joinLeft(array('m' => 'vehicle_status'),'m.vehicle_id=v.id',array('status'))
						->where('m.status = 10');
		return $vehicle->fetchAll($select);
	}
	...
	*/
	...
	/**
	*	Check if external number has already used in system by another vehicle.
	*
	*	@param integer $externalNumber -> vehicle's external number
	*	@param integer $vehicleId -> vehicle's id
	*	@access public
	*	@return boolean
	*/
	private function checkedExternalNumber($externalNumber, $vehicleId = 0)
	{
		$vehicleHistoric = new Application_Model_DbTable_VehicleHistoric();
		$vehicleHistoricRow = $vehicleHistoric->fetchAll($vehicleHistoric->select()->where('external_number = ?',$externalNumber));
		if(!count($vehicleHistoricRow))
		{
			return true;
		}
		if($vehicleId)
		{
			foreach ($vehicleHistoricRow as $historic) 
			{
				if($vehicleId == $historic->vehicle_id)
				{
					return true;
				}
			}
	...
	/**
	*	Accept down vehicle in SETOP changing status to 4 and historic date to now.
	*
	*	@param integer $vehicleId -> vehicle's id
	*	@access public
	*	@return Zend_Table_Db_Row
	*/
	public function reviewAcceptDown($vehicleId)
	{
		...
		{
			$vehicleHistoricRow->end_historic_date = new Zend_Db_Expr('NOW()');
			return $vehicleHistoricRow->save();
		}*/
	}

	/**
	*	Deny down vehicle in SETOP.
	*
	*	@param integer $vehicleId -> vehicle's id
	*	@access public
	*	@return null
