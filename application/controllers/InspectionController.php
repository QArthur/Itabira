...
class InspectionController extends Zend_Controller_Action
{
    ...
    public function init()
    {
      ...
      $this->view->listVehiclesDown = count($inspection->listVehiclesDown());
    }
          ...
          if($data['document'] == 'inspection' || $data['document'] == 'notification'){
            if(isset($_FILES['file']) && !is_null($_FILES['file']))
            {
              if($vehicle->saveDocument($_FILES,$data,$vehicleId) && ($data['date_inspection'] != '' || $data['date_notification'] != '')){
                ...
                $inspection = new Application_Model_Inspection();
                $inspection->saveLogInspection($vehicleId,4);
    ...
    public function saveAllAction()
    {
     	if ( $this->getRequest()->isPost()) 
      {
        ...
        if($data['block'] == 1){
          if($vehicle->changeStatus($data['vehicleId'],14))
          {
            $inspection->saveLogInspection($data['vehicleId'],2);
            $is_new = $vehicle->vehicleIsNew($data['vehicleId']);
            if($is_new){
              $vehicle->changeStatusVehicleNew($data['vehicleId'],0);  
            }
            $this->redirect('/inspection/protocol-block');
          }
        }
        else{
          if($vehicle->changeStatus($data['vehicleId'],4))
          {
            $inspection->saveLogInspection($data['vehicleId'],1);
            $is_new = $vehicle->vehicleIsNew($data['vehicleId'],0);
            if($is_new){
