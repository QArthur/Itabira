...
class QcoController extends Zend_Controller_Action
{
    ...
    public function editAction()
    {
      try
      {
        ...
        else if($save == 'failed' || (isset($result) && $result == false))
        {
    ...
    public function newQhAction()
    {
      if( $this->getRequest()->isPost() ) 
      {
        $qco = new Application_Model_Qco();
        $data = $this->getRequest()->getPost();
        if($qco->checkQH($_FILES,$data))
        {
          $qh = new Application_Model_QcoQH();
          $this->view->qh = $qh->loadQH($_FILES);
          $this->view->qcoFleet = $qco->returnFleetById($data['qco_id']);
          // print_r($this->view->qh);exit;
          $this->view->serializedQh = serialize($this->view->qh);
          $this->view->qcoId = $data['qco_id'];
        }
        else
        {
      ...
      {
        $this->_redirect('/qco/view/save/failed');
      }
    }
    ...
    public function acceptQhAction()
    {
      if ( $this->getRequest()->isPost() ) 
      {
        $data = $this->getRequest()->getPost();
        $data['serializedQh'] = unserialize($data['serializedQh']);
        $qco = new Application_Model_Qco();
        $qcoQH = new Application_Model_QcoQH();
        $newQcoId = $qco->cloneQCO($data['qcoId']);
        $qcoQH->saveNewQH($newQcoId, $data['qcoId'], $data);
        $this->_redirect('/qco/edit/id/'.$newQcoId.'/save/success');
      }
      else
      {
        $this->_redirect('/qco/view');
      }
    }
