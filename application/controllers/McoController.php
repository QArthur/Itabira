...
class McoController extends Zend_Controller_Action
{
    ...
    public function accreditPassengerAction()
    {
      ...
      if ( $this->getRequest()->isPost() ) 
      {
        ...
        {
           $this->_redirect('/mco/analytics-diff/id/'.$mcoId.'/date/'.$date.'/save/success');
        }
        else
        {
          $this->_redirect('/mco/analytics-diff/id/'.$mcoId.'/date/'.$date.'/save/error');
        }
      }
      $this->view->list = $mcoData->returnPassengerDiff($id);
      $this->view->list2 = $mcoId;
      $this->view->date = $date;
    }
    ...
    public function editLostLogAction()
    {
      ...
      if ( $this->getRequest()->isPost() ) 
      {
        ...
        {         
            $this->_redirect('/mco/analytics-result/id/'.$mcoId.'/date/'.$date.'/save/success');
        }
        else
        {    
            $this->_redirect('/mco/analytics-result/id/'.$mcoId.'/date/'.$date.'/save/error');
        }
      }
      $this->view->list = $mcoData->returnJourneysId($id);
      $this->view->date = $date;
      $this->view->mcoId = $mcoId;
    }
  ...
  public function reportOvercrowdedAction(){
    ...
    public function consortiumMcoAction()
    {
      try {
      {
      $mco = new Application_Model_Mco();
    ...
    public function consortiumMcoAnalyticsAction()
    {
      $mco = new Application_Model_Mco();
      $pagination = new Application_Model_Pagination();
      $page = $this->getRequest()->getParam('page');
    ...
    public function consortiumMcoAnalyticsByDayAction()
    {
      try {
      $date = $this->getRequest()->getParam('date');
      $this->view->date = $date;  
    ...
    public function consortiumMcoJourneyChangeAction()
    {
      try{
        $pagination = new Application_Model_Pagination();
        $mcoData = new Application_Model_McoData();
