...
class FleetController extends Zend_Controller_Action
{
    ...
    public function init()
    {
      ...
      if($this->view->institution == 1)
      {
        ...
        $this->view->vehiclesReviewed = count($vehicle->reviews());
        $this->view->vehiclesReviewedAdd = count($vehicle->reviewsAdd());
        $this->view->vehiclesEdited = count($vehicle->reviewsEdited());
        $this->view->vehiclesDown = count($vehicle->vehiclesDown());
      }
    }
      ...
      try{
        ...
        if ( $this->getRequest()->isPost() ) 
        {
          ...
          {			 
            ...
            $inspection->saveLogInspection($vehicleId,3);
		...
		//echo"nnnnnn".$this->view->completed;exit;
        if($this->view->institution == 3 && $this->view->completed && $vehicleStatusRow->status != 4 && $vehicleStatusRow->status != 2 && $vehicleStatusRow->status != 3)
        {
          $this->view->result = 6;
        }
      }catch(Zend_Exception $e){
    ...
    public function reviewAddAction()
    {
      ...
      if ( $this->getRequest()->isPost() ) 
      {
        ...
        $vehicle->changeStatus($data['id'],$status);
        $vehicle->saveLog($data['id'], $status);
        return $this->redirect('/fleet/review-add/success');
      }
    ...
    public function removeFileAction()
    {
      try{
        ...
        $fileDir = APPLICATION_PATH . '/../public/upload/vehicle/'.$vehicleId.'/'.$file;
     ...
     if ( $this->getRequest()->isPost()) 
      {
        ...
        {
          //$vehicle->changeStatusVehicleNew($data['vehicleId'],$status);
    ...
    public function printCertificateAction()
    {
      try{
        ...
        $vehicleStatus = new Application_Model_DbTable_VehicleAskCrv();
        $vehicleStatusRow = $vehicleStatus->fetchAll($vehicleStatus->select()->where('vehicle_id = ?',$id));
        foreach ($vehicleStatusRow as $vehicleRow) 
        {
            $vehicleRow->status = 0;
            $vehicleRow->save();
        }
    ...
    public function addDocumentsAction()
    {
      ...
      if ( $this->getRequest()->isPost() ) 
      {
        ...
        { 
          ...
          {
            //salvar na inspection para aparecer na listagem
            $vehicle->saveInspection($data,$this->view->vehicleId);
            $this->view->save = 'success';
          }
          else
          {
    ...
    public function printInspectionAction()
    {
      try{
        $id = $this->getRequest()->getParam('id');
        $this->_helper->layout()->setLayout('ajax');
        header('Content-Type: application/pdf');
        $printInspection = new Application_Model_VehiclePdfInspection();
        $pdf = $printInspection->createPdf($id);
        echo $pdf->render();
      }catch(Zend_Exception $e){
        $this->view->error = true;
      }
    ...
    public function reportAction()
    {
      try{ 
        ///$this->_helper->layout()->setLayout('ajax');
        $vehicle = new Application_Model_Vehicle();       
        $vehicleaux= $vehicle->returnAvgYearChassi();
        $this->view->list=$vehicleaux;
